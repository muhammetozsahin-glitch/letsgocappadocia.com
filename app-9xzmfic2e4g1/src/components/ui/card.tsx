...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-card text-card-foreground flex flex-col gap-5 rounded-[calc(var(--radius)+0.15rem)] border py-5 shadow-card transition-shadow duration-200 hover:shadow-hover",
        className
      )}
      {...props}
    />
